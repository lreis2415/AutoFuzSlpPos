...
def PreProcessing(model):
    ...
    if D8StreamThreshold == 0:
        ...
        maxAccum, minAccum, meanAccum, STDAccum = GetRasterStat(
            D8ContriArea)  # print maxAccum, minAccum, meanAccum, STDAccum
        ...
        if outlet is not None:
            TauDEM.DropAnalysis(demfil, D8FlowDir, D8ContriArea, D8ContriArea, outletM, minthresh, maxthresh, numthresh,
                                logspace, inputProc, drpFile, mpiexeDir=mpiexeDir, exeDir=exeDir, hostfile=hostfile)
            drpf = open(drpFile, "r")
    ...
    if model == 0:
        ...
        TauDEM.DinfDistUpToRidge(DinfFlowDir, demfil, DinfSlp, propthresh, DinfUpStat, DinfUpMethod, 'false', inputProc,
                                 DinfDistUp, rdg=rdgsrc, mpiexeDir=mpiexeDir, exeDir=exeDir, hostfile=hostfile)
        TauDEM.DinfDistDown(DinfFlowDir, demfil, D8Stream, DinfDownStat, 'Vertical', 'false', DinfDistDownWG, inputProc,
                            DinfDistDown_V, mpiexeDir=mpiexeDir, exeDir=exeDir, hostfile=hostfile)
