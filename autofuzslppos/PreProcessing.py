...
from pygeoc.TauDEM import TauDEMWorkflow, TauDEM_Ext
...
def pre_processing(cfg):
    ...
    if cfg.flow_model == 1:  # Dinf model, extract stream using the D8 threshold
        TauDEM_Ext.dinfflowdir(cfg.proc, cfg.pretaudem.filldem, cfg.pretaudem.dinf,
                               cfg.pretaudem.dinf_slp, cfg.ws.pre_dir, cfg.mpi_dir, cfg.bin_dir)
        if cfg.valley is None or not FileClass.is_file_exists(cfg.valley):
            ...
            TauDEM_Ext.threshold(cfg.proc, cfg.pretaudem.dinfacc_weight,
        ...
        TauDEM_Ext.dinfdistdown(cfg.proc, cfg.pretaudem.dinf, cfg.pretaudem.filldem,
        ...
        # calculate Height Above the Nearest Drainage (HAND)
        TauDEM_Ext.d8distdowntostream(cfg.proc, cfg.pretaudem.d8flow,
    ...
    if cfg.rpi_method == 1:  # calculate RPI based on hydrological proximity measures (Default).
        if cfg.flow_model == 0:  # D8 model
            TauDEM_Ext.d8distdowntostream(cfg.proc, cfg.pretaudem.d8flow,
                                          ...
                                          cfg.ws.pre_dir, cfg.mpi_dir, cfg.bin_dir,
                                          cfg.log.preproc, cfg.log.runtime, cfg.hostfile)
            TauDEM_Ext.d8distuptoridge(cfg.proc, cfg.pretaudem.d8flow,
            ...
            TauDEM_Ext.dinfdistdown(cfg.proc, cfg.pretaudem.dinf, cfg.pretaudem.filldem,
                                    ...
                                    cfg.ws.pre_dir, cfg.mpi_dir, cfg.bin_dir,
                                    cfg.log.preproc, cfg.log.runtime, cfg.hostfile)
            TauDEM_Ext.dinfdistuptoridge(cfg.proc, cfg.pretaudem.dinf,
                                         ...
                                         cfg.ws.pre_dir, cfg.mpi_dir, cfg.bin_dir,
                                         cfg.log.preproc, cfg.log.runtime, cfg.hostfile)
        TauDEM_Ext.simplecalculator(cfg.proc, cfg.pretaudem.dist2stream,
    ...
    if cfg.rpi_method == 0:  # calculate RPI based on Skidmore's method
        if cfg.ridge is None or not FileClass.is_file_exists(cfg.ridge):
            ...
            if cfg.flow_model == 1:  # D-inf model
                ...
                elevfile = cfg.pretaudem.dist2stream_v
            TauDEM_Ext.extractridge(cfg.proc, angfile, elevfile, cfg.ridge,
                                    cfg.ws.pre_dir, cfg.mpi_dir, cfg.bin_dir,
                                    cfg.log.preproc, cfg.log.runtime, cfg.hostfile)
        TauDEM_Ext.rpiskidmore(cfg.proc, cfg.valley, cfg.ridge,
    ...
    TauDEM_Ext.curvature(cfg.proc, cfg.pretaudem.filldem,
