...
public:
    ...
    virtual bool globalToLocal(int globalX, int globalY, int &localX,
                               int &localY) = 0; // Converts global coordinates (for the whole grid) to local coordinates (for this partition).  Function returns TRUE only if the coordinates are contained in this partition.
    virtual void localToGlobal(int localX, int localY, int &globalX,
                               int &globalY) = 0; // Converts local coordinates (for this partition) to the whole grid.
    ...
    virtual void init(long totalx, long totaly, double dx_in, double dy_in, MPI_Datatype MPIt,
                      short nd) { }  //Takes the total number of rows and columns in the ENTIRE grid to be partitioned,
