...
//				printf("Unhandled tag: %d\n",ifds[index].tag);
    ...
    if (origRasterType == 2)
    {
        ...
        if (rank == 0)
            printf("uses the PixelIsPoint Raster Space convention.  The output files have adjusted \ntop and left edge values for the PixelIsArea default convention.\n");
    ...
    if (noDataDef != 1)
    {
        ...
        else
        {
            ...
            } else if ((sampleFormat == 1) && (dataSizeFileIn == 8))
            {
                filenodata = new uint64_t;
                *((uint64_t *) filenodata) =
                        ((uint64_t) (4294967296)) * ((uint64_t) (4294967295)) + ((uint64_t) (4294967295));
                *((float *) nodata) = (float) (*((uint64_t *) filenodata));
            } else if ((sampleFormat == 2) && (dataSizeFileIn == 1))
            {
                filenodata = new int8_t;
...
//	long lastStripIndex = (long) floor((ystart + numRows)/tileLength ); //index into tiff strip arrays of the last strip to be read
        ...
        long lastStripIndex =
                (ystart + numRows - 1) / tileLength; //index into tiff strip arrays of the last strip to be read
        long firstStripFirstRowIndex =
                ystart - (firstStripIndex * tileLength); //index within the first tiff strip of the first row to be read
        long lastStripLastRowIndex = ystart + numRows - 1 - (lastStripIndex *
                                                             tileLength); //index within the last tiff strip of the last row to be read
        ...
        { //7/24/13 DGT added condition of firstStripIndex to account for partitions entirely within a strip
    ...
    {  // tiles.  The strategy is to read one row at a time reading the parts
        ...
        tilesAcross = (totalX - 1) / tileWidth +
                      1;  // DGT 7/8/10 - was tilesAcross=totalX/tileWidth+1 which failed if totalX was a multiple of tileWidth
