...
//	printf("Before angwrite rank: %d\n",rank);
        ...
        if (rank == 0)
        {
            ...
            printf("Processors: %d\nRead time: %f\nCompute time: %f\nWrite time: %f\nTotal time: %f\n",
                   size, headerRead + dataRead, computeSlope + computeFlat, writeSlope + writeFlat, total);
        }
    }
    //MPI_Barrier(MCW);
    MPI_Finalize();
...
void SET2(int I, int J, float *DXX, float DD, tdpartition *elevDEM, tdpartition *elev2, tdpartition *flowDir,
          tdpartition *dn)
{
    float SK[9];
    float ANGLE[9];
    ...
    for (K = 1; K <= 8; K++)
    {
        dn->getData(J + J1[K], I + I1[K],
                    tempShort1);//Check each square to see if it is in the flat.  If it is in the flat, use artifical elevations if not use real elevations
        dn->getData(J + J2[K], I + I2[K], tempShort2);//dn = 0 if it is not in the flat.  dn = 1 if in flat.
        if (tempShort1 <= 0 && tempShort2 <= 0)
        { //Both E1 and E2 are outside the flat get slope and angle
        ...
        {//E2 is out side of the flat and E1 is inside the flat, use DEM elevations
            float a = elevDEM->getData(J, I, tempFloat);
            //float b=elevDEM->getData(J+J1[K],I+I1[K],tempFloat);
            float c = elevDEM->getData(J + J2[K], I + I2[K], tempFloat);
            if (a >= c)
        ...
        {//Both E1 and E2 are in the flat. Use artificial elevation to get slope and angle
            short a, b, c;
        ...
        for (iflat = 0; iflat < nflat; iflat++)
        {
            ...
            for (k = 1; k <= 8; k++)
            {
                if (dontCross(k, i, j, flowDir) == 0)
                {
                    jn = j + d2[k];
                    in = i + d1[k];
                    elevDiff = elevDEM->getData(i, j, tempFloat) -
                               elevDEM->getData(in, jn, tempFloat);// get elevations and elevation difference.
                    flowDir->getData(in, jn, tempFloat); //get current flow direction...
                    if (elevDiff >= 0 && tempFloat >=
                                         0.0) //adjacent cell drains and is equal or lower in elevation so this is a low boundary
            ...
            if (!doNothing)
            {//if I still have to do something...
        ...
        for (iflat = 0; iflat < nflat; iflat++)
        {
            ...
            for (k = 1; k <= 8; k++)
            {
                if (dontCross(k, i, j, flowDir) == 0)
                {
                    ...
                    if (elevDiff >= 0 && tempFloat >=
                                         0.) //adjacent cell drains and is equal or lower in elevation so this is a low boundary
                        doNothing = true;
                    else if (elevDiff == 0) //if neighbor is in flat
                    if (elev2->getData(in, jn, tempShort) >= 0 &&
                        elev2->getData(in, jn, tempShort) < st) //neighbor is not being incremented
