...
// SHARE elev to populate the borders
	...
	//--Calculate Streams--
  	  for(y=-1; y < elevny; y++){
		  ...
		  {
			  emin =elev->getData(x,y,floatTemp1); 
			  iomax=0;
			  jomax=0;   
			  bound=0;  					/*  .false.  */
			
									/*  --FIRST PASS FLAG MIN ELEVATION IN GROUP OF FOUR  */
			  for(ik=0; ik<2; ik++)
				  for(jk=1-ik; jk < 2; jk++)
				  {
					  if(elev->isNodata(x+jk,y+ik))
						 bound= 1; 
					  else if(elev->getData(x+jk,y+ik,floatTemp1) < emin)
					  {
						  emin=floatTemp1;
						  iomax=ik;
						  jomax=jk;
			 ...
			 ss->setData(x+jomax,y+iomax,(short)0);
			  ...
			  if(bound == 1)
			  {
				...
				for(ik=0; ik < 2; ik++)
				  for(jk=0; jk< 2; jk++)
				  {
					 if(elev->getData(x+jk,y+ik,floatTemp1) == emin)
					 {
						ss->setData(x+jk,y+ik,(short)0);
					 }
					 if (minElev != MISSINGFLOAT)
					 {
						 if(elev->getData(x+jk,y+ik,floatTemp1) < minElev)
							 ss->setData(x+jk,y+ik,(short)0);
					 }
