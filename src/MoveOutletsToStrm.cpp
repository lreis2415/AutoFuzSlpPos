...
int outletstosrc(char *pfile, char *srcfile, char *outletshapefile, char *movedoutletshapefile, int maxdist)
{
    ...
    MPI_Init(NULL, NULL);
    {
        ...
        srcData->localToGlobal(0, 0, srcxstart,
                               srcystart);  //  DGT here no typecast - but 2 lines down there is typecast - why
        ...
        while (!done)
        {
            for (i = 0; i < nxy; i++)
            {
                ...
                if (flowData->isInPartition(tx, ty) && dist_moved[i] >= 0)
                {
                    ...
                    if (srcData->isNodata(tx, ty) || td < itresh)
                    {  //  If not on stream.  The rule is any value >= itresh is a stream, less or no data is not stream
                        //move the outlet
                        ...
                        if (dirn >= 1 && dirn <= 8 && dist_moved[i] < maxdist)
                        {
                            ...
                            if (nextx >= 0 && nexty >= 0 && nextx < pTotalX && nexty < pTotalY)
                            {  // If is within global domain
                                outletsX[i] = nextx;
                                outletsY[i] = nexty;
                                //ismoved[i]=1;
                                dist_moved[i]++;
            ...
            for (i = 0; i < nxy; i++)
            {
                ...
                flowData->globalToLocal(toutletsX[i], toutletsY[i], txn,
                                        tyn);  //DGT with single partition this is being passed uninitialized information
                flowData->globalToLocal(outletsX[i], outletsY[i], tx, ty);
                if (flowData->isInPartition(txn, tyn) && !(part_has[i] == rank))
                {  // if an outlet not in partition is now in partition - grab it
        ...
        if (!rank)
        {
            for (i = 0; i < nxy; i++)
            {
                ...
                if (dist_moved[i] <= 0)
                {  // DGT changed condition to dist_moved <=0 because original values kept whenever not moved for whatever reason
                    xnode[i] = origxnode[i];
                    ynode[i] = origynode[i];
                }
            }
