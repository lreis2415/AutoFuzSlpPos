							...
							if(dts->isNodata(in,jn)) dts->setToNodata(i,j);
								...
								else if (statmethod == 2)
								{
									if (first)
									{
										...
										if(dist[k]+ dtss < distr) distr = distk+ dtss;
								}
							}
						}
					}
	...
	MPI_Init(NULL,NULL);
	{
		...
		while(!finished)
		{
			while(!que.empty())
			{
					...
					if(!flowdir->hasAccess(in,jn) || flowdir->isNodata(in,jn)){
						...
						if (angle - k == 4 || angle - k == -4)
						{
							...
							else if (felData->isNodata(in,jn))
							{
								...
								{
									sump = sump + 1;
									distrh = distrh + dtss + dist[k];
									distr = distr + distk+ dtsvv;
								}
								else if (statmethod == 1)
								{
				...
				if (statmethod == 0 && sump >= 1){
					dts->setData(i,j,(float)(distrh/sump));
					dtsv->setData(i,j,(float)(distr/sump));
				}
				else{
		...
		for(j=0; j<ny; j++) {
			for(i=0; i<nx; i++) {
				...
				else if(!dts->isNodata(i,j))
				{
					...
					//distrh=sqrt(distrh*distrh+distr*distr);
					//if(distrh>10000 || distr > 10000 || distrh<-10000 || distr < -10000  )printf("h:%f,v:%f,\n",distrh,distr);
					dts->setData(i,j,sqrt(distrh*distrh+distr*distr));
					//if(distrh > 1000 )printf("p:%f\n",dts->getData(i,j,distrh));
				}
			}
		}
		//Stop timer
