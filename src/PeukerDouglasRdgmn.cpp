...
int peukerdouglas(char *felfile,char *ssfile,float minElev,float *p);
...
//		printf("You are running %s with the specific file names option.\n", argv[0]);
    ...
    if( (err=peukerdouglas(felfile,ssfile,minElev,p)) != 0)
...
errexit:
   ...
   printf("-ss <streamsource> [-elev <minElevation>] [-par <weightMiddle> <weightSide> <weightDiagonal>]\n");
   ...
   printf("<minElevation> is the lowest elevation for ridge.");
