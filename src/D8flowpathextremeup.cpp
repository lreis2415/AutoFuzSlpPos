...
//  Begin timer
 ...
 // printf("%d %d\n",rank,numOutlets);
  //for(int i=0; i<numOutlets; i++)printf("%g, %g\n",x[i],y[i]);
	...
	node temp;
	queue<node> que;
	initNeighborD8up(neighbor,flowData,&que,nx,ny,useOutlets,outletsX,outletsY,numOutlets); // que contains locations that no neighbors drain to them
	finished = false;
	...
	while(!finished) {
		while(!que.empty()){
			...
			if(flowData->isInPartition(i,j)){
				//  Initial ssa value is the value of saData at the location
