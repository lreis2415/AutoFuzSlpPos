...
//Set positive flow directions of elevDEM
...
int setdird8(char *demfile, char *pointfile, char *slopefile, char *flowfile, int useflowfile)
{
    ...
    MPI_Init(NULL, NULL);
    {
        ...
        if (useflowfile == 1)
        {
            ...
            imposedflow = CreateNewPartition(flow.getDatatype(), flow.getTotalX(), flow.getTotalY(), flow.getdx(),
                                             flow.getdy(), flow.getNodata());
            ...
            for (j = 0; j < elevDEM->getny(); j++)
            {
                for (i = 0; i < elevDEM->getnx(); i++)
                {
                    ...
                    if (imposedflow->isNodata(i, j) || !imposedflow->hasAccess(i - 1, j) ||
                        !imposedflow->hasAccess(i + 1, j) ||
