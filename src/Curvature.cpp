...
int Curvature(char *demfile,char *profcfile,char *plancfile,char *horizcfile,char *unspherfile,char *meancfile,char *maxcfile,char *mincfile,bool calprof ,bool calplan,bool calhoriz,bool calunspher,bool calmeanc,bool calmaxc, bool calminc)
{
	MPI_Init(NULL,NULL);
	{
		int rank,size;
		...
		double dx = demf.getdx();
		double dy = demf.getdy();
...
#pragma omp for
		for (j = 0; j < ny; j++)
		{
			for (i = 0; i < nx; i++)
			{
				...
				if (dem->isNodata(i,j) || !dem->hasAccess(i-1,j) || !dem->hasAccess(i+1,j) || !dem->hasAccess(i,j-1) || !dem->hasAccess(i,j+1))
				{
					if(calprof)
					...
					if (hasNodata)
					{
						...
						p = (z[2] + z[1] + z[8] - z[4] - z[5] - z[6]) / (6 * dx);
						q = (z[4] + z[3] + z[2] - z[6] - z[7] - z[8]) / (6 * dx);
						r = (z[4] + z[2] + z[5] + z[1] + z[6] + z[8] - 2 * (z[3] + z[0] + z[7])) / (3 * cellsize2);
						s = (-z[4] + z[2] + z[6] - z[8]) / (4 * cellsize2);
						t = (z[4] + z[3] + z[2] + z[6] + z[7] + z[8] - 2 * (z[5] + z[0] + z[1])) / (3 * cellsize2);
						if (p == 0 && q == 0)
						{
							if(calprof)
								prof->setData(i,j,(float)0.0);
							if(calplan)
								plan->setData(i,j,(float)0.0);
							if(calhoriz)
								horiz->setData(i,j,(float)0.0);
						}
						else{
							if(calprof)
								prof->setData(i,j,-(r * p * p + t * q * q + 2 * p * q * s) / ((p * p + q * q) * pow((1 + p * p + q * q),(float)1.5)));
							if(calplan)
								plan->setData(i,j,-(r * q * q + t * p * p - 2 * p * q * s) / (pow((p * p + q * q),(float)1.5)));
							if(calhoriz)
								horiz->setData(i,j,-(q * q * r - 2 * p * q * s + p * p * t) / ((p * p + q * q) * pow((1 + p * p + q * q),(float)1.5)));
						}
						if (calunspher || calmeanc || calmaxc || calminc)
						{
							float meancurv,unspherV;
							meancurv = -((1 + q * q) * r - 2 * p * q * s + (1 + p * p) * t) / (2 * pow((1 + p * p + q * q),(float)1.5));
							unspherV = sqrt(pow((r * sqrt((1 + q * q) / (1 + p * p)) - t / sqrt((1 + q * q) / (1 + p * p))),(float)2.0) / (1 + p * p + q * q) + pow((p * q * r * sqrt((1 + q * q) / (1 + p * p)) - 2 * sqrt((1 + q * q) * (1 + p * p)) * s + p * q * t / sqrt((1 + q * q) / (1 + p * p))),(float)2.0)) / (2 * pow((1 + p * p + q * q),(float)1.5));
							if (calmeanc)
