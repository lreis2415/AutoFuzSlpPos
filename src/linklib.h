...
void setLinkInfo(long **LinkIdU1U2DMagShapeid, double **LinkElevUElevDLength, double **PointXY, float **PointElevArea,
                 tdpartition *elev, tiffIO *elevIO);
...
streamlink *takeOut(long Id)
{
    ...
    {// not the first llnode in the list
...
bool sendLink(long Id, int dest)
{
    ...
    int bsize = toSend->numCoords * sizeof(point) * 2 +
                MPI_BSEND_OVERHEAD;  // Experimentally this seems to need to have >47 added for overhead
    buf = new point[bsize];
...
void SendAndReciveLinks(int nx, int ny, tdpartition *idGrid, tdpartition *contribs, tdpartition *flowDir,
                        tdpartition *src)
{
    int linksSent = 0;
    int linksRecv = 0;
    ...
    {//even send
        ...
        {
            for (i = 0; i < nx; i++)
            {//count links to send down
                if (!contribs->isNodata(i, -1) && contribs->getData(i, -1, tempShort) < 0)
                {//i am on the boarder and I am negitive, check who points to me and send there id.
        ...
        if (rank != size - 1)
        {
            for (i = 0; i < nx; i++)
            {//count links to send down
                if (!contribs->isNodata(i, ny) && contribs->getData(i, ny, tempShort) < 0)
                {//i am on the boarder and I am negitive, check who points to me and send there id.
                    int p = flowDir->getData((long) i - 1, (long) ny - 1, tempShort);
                    if (i > 0 && flowDir->getData((long) i - 1, (long) ny - 1, tempShort) == 8 &&
                        src->getData(i - 1, ny - 1, tempShort) == 1 && idGrid->getData(i - 1, ny - 1, tempLong) >= 0)
                    {
                        toSend++;
            ...
            {//count links to send up
                ...
                {//i am on the boarder and I am negitive, check who points to me and send there id.
                    ...
                    if (i > 0 && flowDir->getData((long) i - 1, (long) ny - 1, tempShort) == 8 &&
                        src->getData(i - 1, ny - 1, tempShort) == 1 && idGrid->getData(i - 1, ny - 1, tempLong) >= 0)
                    {
                        if (sendLink(idGrid->getData(i - 1, ny - 1, tempLong), rank + 1))
                        {//check to see if sent properly
                            linksSent++;
                        } else
                        {//didn't send right.
...
/*
	...
	*/
            long i = 0;
            double cellarea = elev->getdx() * elev->getdy();
            for (i = 0; i < current->data->numCoords; i++)
            {
                ...
                elevIO->globalXYToGeo(current->data->coord.front().x, current->data->coord.front().y,
                                      PointXY[pointsSoFar][0], PointXY[pointsSoFar][1]);
