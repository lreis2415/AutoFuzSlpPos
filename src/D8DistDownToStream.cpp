...
int distgrid(char *pfile, char *felfile, char *srcfile, char *distfile, int typemethod, int thresh)
{
    ...
    {
        case 0:
            er = hdisttostrmgrd(pfile, srcfile, distfile, thresh);
            break;
        case 1:
            er = vdroptostrmgrd(pfile, felfile, srcfile, distfile, thresh);
            break;
        case 2:
            er = pdisttostrmgrd(pfile, felfile, srcfile, distfile, thresh);
            break;
        case 3:
            er = sdisttostrmgrd(pfile, felfile, srcfile, distfile, thresh);
            break;
    }
    return (er);
}
...
int hdisttostrmgrd(char *pfile, char *srcfile, char *distfile, int thresh)
{
    ...
    {  //  All code within braces so that objects go out of context and destruct before MPI is closed
        ...
        float dist[9];
        for (i = 1; i <= 8; i++)
            dist[i] = sqrt(d1[i] * d1[i] * (float) dx * (float) dx + d2[i] * d2[i] * (float) dy * (float) dy);
        ...
        while (!finished)
        {
            while (!que.empty())
            {
                ...
                else
                {
                    ...
                    if (fdarr->isNodata(in, jn))fdarr->setToNodata(i, j);
                        ...
                        fdarr->setData(i, j, dist[k] + fdarr->getData(in, jn, tempFloat));
                }
            ...
            //Check if done
            finished = que.empty();
            finished = (bool)fdarr->ringTerm(finished);
        }
        //Stop timer
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (fel->isNodata(i, j) || fel->isNodata(in, jn))
                        {
                            ...
                            fdarr->setData(i, j, elev - elevn + fdarr->getData(in, jn, tempFloat));
...
//		float tempFloat;
        ...
        while (!finished)
        {
            ...
            //Check if done
            finished = que.empty();
            finished = (bool)fdh->ringTerm(finished);
        }
            ...
            while (!que.empty())
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        fdsv += sqrt((elev - elevn) * (elev - elevn) + dist[k] * dist[k]);
