...
{/// ksmooth function in R
	...
	float cutoff = 0.0, num, den, x0, w;
	/* bandwidth is in units of half inter-quartile range. */
	if(kern == 1) {bw *= 0.5f; cutoff = bw;}
	if(kern == 2) {bw *= 0.3706506f; cutoff = 4.0f*bw;}
	while(x[imin] < xp[0] - cutoff && imin < n) imin++;
	for(int j = 0; j < np; j++) {
		num = den = 0.0;
		x0 = xp[j];
...
int Bigauss_esti_moment(vector<float> &x,vector<float> &y,float powerIdx,vector<float> &sigma_ratio_limit, vector<float> &fit)
{
	...
	else
	{
		...
		else
		{
			...
			{
				pair<vector<float>,vector<int>> d_less_0_ = which(d,0,0.0f); /// operator 0 means less than value, while 1 means greater than.
				vector<float> d_less_0 = d_less_0_.first; 
				pair<vector<float>,vector<int>> d_greater_0_ = which(d,2,0.0f); /// operator 2 means greater_equal than, and 3 means less_equal than
		...
		{
			float sumY = accumulate(y.begin(),y.end(),0.0f);
			float sumProductXY = inner_product(validX.begin(),validX.end(),y.begin(),0.0f);
			m = sumProductXY / sumY;
			float tempv = 0.0f;
			for (i = 0; i < num; i++)
...
int BiGaussianMix(vector<float> &x, vector<float> &y, vector<float> &sigma_ratio_limit,float bandwidth,float powerIdx,int esti_method,float eliminate,float epsilon, int max_iter, vector<vector<float> > &fit_results)
{
...
int Bigauss_esti_em(vector<float> &old_x,vector<float> &old_y,int max_iter, float epsilon, float powerIdx,vector<float> &sigma_ratio_limit, vector<float> &fit)
{
	 ...
	 //program.
	pair<vector<float>, vector<int>> sel = which(old_y,1,1e-10);
	...
	while(change > epsilon && n_iter < max_iter)
	{
		...
		if (n_iter == 1)
		{
			...
			if (isNA(sigma.second) && !isNA(sigma.first))
				sigma.second = sigma.first / 10.0f;
		}
...
float solveBreakPoint(vector<float> &x, vector<float> &y, float a, float sigma1, float sigma2)
{
	...
	float tempSum1 = 0.0f, tempSum2 = 0.0f;
	for (int i = 0; i < x.size(); i++)
	{
		float tempV;
		if (x[i] < a)
			tempV = y[i] * 1.0f/sigma1;
		else
			tempV = y[i] * 1.0f/sigma2;
		tempSum1 += x[i]*tempV;
		tempSum2 += tempV;
	}
...
pair<float,float> solveSigma(vector<float> &x, vector<float> &y, float a)
{
	...
	if (u < ZERO)
		...
		sigma1 = u/y_sum*(pow((v/u),1.f/3.f)+1);
	if (v < ZERO)
		sigma2 = MISSINGFLOAT;
	else
		sigma2 = v/y_sum*(pow((u/v),1.f/3.f)+1);
...
pair<vector<float>, vector<int>> which(vector<float> &x, int op, float v)
{
	...
	case 0:
		{
			it = copy_if(x.begin(),x.end(),tempVector.begin(),bind2nd(less<float>(),v));
			break;
		}
	case 1:
		{
			it = copy_if(x.begin(),x.end(),tempVector.begin(),bind2nd(greater<float>(),v));
			break;
		}
	case 2:
		{
			it = copy_if(x.begin(),x.end(),tempVector.begin(),bind2nd(greater_equal<float>(),v));
			break;
		}
	case 3:
		{
			auto it = copy_if(x.begin(),x.end(),tempVector.begin(),bind2nd(less_equal<float>(),v));
			break;
		}
	}*/
}
...
float STDcal(float *values, int num, bool flag, float ptv)
{
	...
	for (i = 0; i < num; i++)
	{
		if (flag){ // if flag is true, calculate values greater equal than ptv
			if(values[i] >= ptv)
	...
	float mean = accumulate(calValue.begin(),calValue.end(),0.0);
	mean /= calValue.size();
	float sigma = 0.0;
...
pair<int,int> findValue(vector<float> valueVector, float v)
{
	...
	float minVdiff = -1*MISSINGFLOAT, maxVdiff = MISSINGFLOAT;
	int minIdx = -1, maxIdx = -1;
	for (int i = 0; i < valueVector.size(); i++)
	{
		float tempv = v -valueVector[i];
		if(tempv <= minVdiff && tempv > 0.0f && minIdx < i)
		{
			minVdiff = tempv;
			minIdx = i;
		}
		if (tempv >= maxVdiff && tempv < 0.0f && maxIdx < i)
		{
			maxVdiff = tempv;
			maxIdx = i;
		}
