                     ...
                     int concheck)
{
    ...
    MPI_Init(NULL, NULL);
    {
        ...
        srcData = CreateNewPartition(src.getDatatype(), totalX, totalY, dx, dy, src.getNodata());
        ...
        while (!finished)
        {
            while (!que.empty())
            {
                ...
                else
                {
                    ...
                    if ((con && concheck == 1) || sump <= 0.)
                        dts->setToNodata(i,
                                         j); // set to no data if contamination and checking, or if there were no down cells with a result
                    else
                    {
        ...
        while (!finished)
        {
            while (!que.empty())
            {
                ...
                else
                {
                    ...
                    if ((con && concheck == 1) || sump <=
                                                  0.)  // set to no data if contamination and checking, or if there were no down cells with a result
                    {
                        dtsh->setToNodata(i, j);
